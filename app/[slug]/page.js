              ...
              {items.map((p) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {p.description && (
                      <div className="text-xs text-white/45 mt-0.5 line-clamp-2">{p.description}</div>
                    )}
        ...
        <div className="px-6">
          ...
          {mode === "reservar" ? (
            ...
            <p className="text-sm text-white/40 text-center py-8">
              Toca un producto de la carta para añadirlo a tu pedido — tu carrito aparecerá abajo a la derecha.
            </p>
          )}
        </div>
      ...
      {showCartDrawer && (
        ...
        >
          ...
          >
            ...
            {cartLines.map((l) => (
              <div key={l.product.id} className="flex items-center justify-between gap-3 py-2 border-b border-white/10 text-sm">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <span className="w-14 text-right text-mustard font-display">
                    {(l.qty * Number(l.product.price)).toFixed(2)}€
                  </span>
                </div>
              </div>
            ))}
            ...
            <h4 className="text-xs font-semibold text-white/50 mb-2">Elige tu mesa</h4>
...
function ProductModal({ product, qty, setQty, canOrder, onClose, onConfirm }) {
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center" onClick={onClose}>
      <div
        className="bg-[#1E332B] w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl max-h-[90vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {product.image_url ? (
          ...
          {canOrder ? (
            <div className="flex items-center gap-3 mt-2">
              ...
              <button onClick={onConfirm} className="flex-1 bg-mustard text-ink font-semibold py-3 rounded-full text-sm">
                {qty === 0 ? "Quitar del pedido" : "Añadir · " + (qty * Number(product.price)).toFixed(2) + "€"}
              </button>
            </div>
          ) : (
...
function Footer({ business }) {
  const socials = [
    ...
    business.facebook && {
      ...
      label: "Facebook",
      icon: (
        <path d="M13.5 21v-7.5h2.5l.4-3H13.5V8.5c0-.9.2-1.5 1.5-1.5h1.6V4.3C16.3 4.2 15.3 4 14.2 4c-2.4 0-4 1.5-4 4.1V10.5H7.7v3h2.5V21h3.3z" />
      ),
    },
    business.website && {
      key: "website",
    ...
    <div className="px-6 mt-8">
      {hasMap && (
        ...
        >
          ...
          <span className="absolute bottom-3 right-3 bg-ink/90 text-xs font-semibold px-3 py-1.5 rounded-full">
            Cómo llegar
          </span>
        </a>
      )}
